...
var apiMemberURL = "http://demo1.dongledongle.com/api/member";
var apiChatURL = "http://demo1.dongledongle.com/api/serviceapi";
...
function fn_FileDownload() {
    ...
    chatContents += '    <link rel=\"stylesheet\" href=\"http://service.dongledongle.com/Content/ThemeDefault/style_client.css\" />\n';
    chatContents += '</head>\n';
    chatContents += '<body>\n';
    chatContents += '    <section id="tmLayerChat">\n';
    chatContents += '        <div class="tmHead">\n';
    chatContents += '            <h2 class="logo"><a href="http://dongledongle.com" class="hidden" target="_blank">TOKMON</a></h2>\n';
    chatContents += '        </div>\n';
    ...
    chatContents += '    <script src="http://service.dongledongle.com/Scripts/tokmon.ui_client.js"></script>\n';
    chatContents += '</body>\n';
    chatContents += '</html>';
...
function fn_eventBind() {
    ...
    $("#tmBtnConfirmName").click(function () {
        ...
        if (isFixedUserName) {
            ...
            //모니터링 변경블록
            chat.invoke("JoinGroupForAdmin", groupName, chatNickName).catch(function (err) {
                return console.error(err.toString());
            });
    ...
    function ChatExit() {
        ...
        $("#tmInfoBar, #tmNotification").show().animate({ top: -80 }, duration, function () {
            $(this).hide();
            $("#tmPopWrap").fadeIn(duration);
        });
        $("#tmInputMessage").prop("disabled", true);
        $("#tmSendMessage").prop("disabled", true);
    ...
    $("#btnAdminLogin").click(function (event) {
        ...
        $.ajax({
            type: "GET",
            url: apiMemberURL + "/?id=" + userid + "&pwd=" + userpwd + "&groupName=" + groupName + "&tokenid=",
            async: false,
            dataType: "json",
...
function fn_init() {
    ...
    if (isMobile.any()) {
        ...
        //    if (fn_isStandAlone()) { // 모바일 여부도 함께 체크하면 됨.
        //        alert("click");
        //        fn_enterFullScreen(document.getElementById("body"));
        //    }
        //});
    }
